...
- (id)initWithProductIdentifiers:(NSSet *)productIdentifiers withCompleteBlock:(IAPbuyProductCompleteResponseBlock)completion {
    self = [self initWithProductIdentifiers:productIdentifiers];
...
- (void)requestProductsWithPid:(NSString*)pid WithCompletion:(IAPProductsResponseBlock)completion {
...
- (void)completeTransaction:(SKPaymentTransaction *)transaction {
    [transaction storageOrderInfoWhenPurched];
...
//    }
    ...
    {
        _buyProductCompleteBlock([transaction orderInfo], transaction);
    }
...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray *)transactions
{
    ...
    {
        NSString *s = [NSString stringWithFormat:@"订单: %@\n", transaction.transactionIdentifier];
        s = [NSString stringWithFormat:@"%@原始订单: %@\n", s, transaction.originalTransaction.transactionIdentifier];
        s = [NSString stringWithFormat:@"%@paymentId: %@\n", s, transaction.payment.applicationUsername];
        s = [NSString stringWithFormat:@"%@订单状态: %@\n", s, transaction];
        [s XLog];
        ...
        switch (transaction.transactionState)
        {
            ...
            default:
                NSLog(@"订单: %@ 状态: %ld", transaction.transactionIdentifier, (long)transaction.transactionState);
                break;
        }
    }
}

- (void)buyProduct:(SKProduct *)productIdentifier withUserId:(NSString*)userId  onCompletion:(IAPbuyProductCompleteResponseBlock)completion {
    
    self.buyProductCompleteBlock = completion;
    
    self.restoreCompletedBlock = nil;
    SKMutablePayment *payment = [[SKPayment paymentWithProduct:productIdentifier] mutableCopy];
